...
func LoadConfig(path string) (*Config, error) {
	...
	if cfg.PollIntervalSeconds <= 0        { cfg.PollIntervalSeconds = 60 }
	if cfg.SlowQueryThresholdMS <= 0       { cfg.SlowQueryThresholdMS = 5000 }
	if cfg.MaxConnectionsThreshold <= 0    { cfg.MaxConnectionsThreshold = 200 }
	if cfg.CPUAlertThresholdPct <= 0       { cfg.CPUAlertThresholdPct = 80 }
	if cfg.MemoryAlertThresholdPct <= 0    { cfg.MemoryAlertThresholdPct = 10 }
	if cfg.DiskIOAlertMS <= 0              { cfg.DiskIOAlertMS = 50 }
	if cfg.ReplicationLagAlertSeconds <= 0 { cfg.ReplicationLagAlertSeconds = 30 }
	if cfg.BackupFullAlertHours <= 0       { cfg.BackupFullAlertHours = 25 }
	if cfg.BackupLogAlertHours <= 0        { cfg.BackupLogAlertHours = 4 }
	if cfg.IndexFragAlertPct <= 0          { cfg.IndexFragAlertPct = 30 }
	if cfg.IndexCheckHours <= 0            { cfg.IndexCheckHours = 6 }
	if cfg.IntegrityCheckHours <= 0        { cfg.IntegrityCheckHours = 24 }
	if cfg.JobFailureLookbackHours <= 0    { cfg.JobFailureLookbackHours = 2 }
	if cfg.AlertCooldownMinutes <= 0       { cfg.AlertCooldownMinutes = 10 }
	if cfg.AnomalyThresholdPct <= 0        { cfg.AnomalyThresholdPct = 30 }
	if cfg.AnomalyMinSamples <= 0          { cfg.AnomalyMinSamples = 10 }
	if cfg.LogMaxSizeMB <= 0               { cfg.LogMaxSizeMB = 50 }
	if cfg.LogMaxFiles <= 0                { cfg.LogMaxFiles = 30 }
	if cfg.DashboardPort <= 0              { cfg.DashboardPort = 8080 }
	if cfg.PrometheusPort <= 0             { cfg.PrometheusPort = 9090 }
	if cfg.LogFile == ""                   { cfg.LogFile = "sql_monitor.log" }
	if cfg.LogLevel == ""                  { cfg.LogLevel = "INFO" }
	if cfg.DiskWarnFreePct <= 0            { cfg.DiskWarnFreePct = 20 }
	if cfg.DiskCritFreePct <= 0            { cfg.DiskCritFreePct = 10 }
